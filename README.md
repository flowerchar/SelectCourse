...
- 管理员编辑课程，学生选课，教师通过**(后端的共性是继承一个父类，然后差异是子类扩展属性|前端的共性是复用一个页面，差异是通过v-if展示)**
- 只有登录页需要校验身份，用不着每一次请求后端服务之前都去数据库鉴别一次身份是否正确(**无意义**且**浪费资源**)。因此在登录鉴别成功之后，把相关信息保存到本次会话。同理前端把校验正确的信息存放到localStorage，避免不必要开销
- 在类上打@RestController @Service @Mapper/@Repostry，就意味着把这些类实例化一个唯一的对象，并且加入到spring容器里（一次性批量把汤圆下锅）
...
- 以json传送的数据要加@Requestbody( RequestParm|PathVariable) axios
- 通过v-if结合level控制权限
- 所有标签里面的class属性都只是控制样式，不涉及代码逻辑
- SELECT * FROM student_info limit 4,3 是从第四行开始(包括这一行)，返回三行数据。(并不是第4页，每页3条数据的意思)mysql索引从零开始。使用PageHelper有两个好处：
  - 不用自己写sql语句的limit
  - 额外返回上下页信息
- 回显
...
> ​	"pageSize":5
>
> }模糊查询 一定程度上RequestParam可以跟PathVariable互换
>
> http://localhost:8888/collegeInfo/page/a?pageNum=1&pageSize=5
>
> http://localhost:8888/collegeInfo/page/,  queryObject

- 在很多场景下，新增跟编辑是复用一个组件（因为数据基本一样），那么怎么区分是新增还是编辑？通过id。**因为新增还没有把数据插入到数据库里面**，那么就不存在id。但是对于编辑，数据是已经从数据库里面返回的，那么一定存在id
- 软编码概念很广泛，包括数据库里面一张表存放另一张表的主键id来实现两张表关联。层与层的**向右关联通过@Autowired**注入，层与**层的向左关联通过return**返回
...
- user是在登录时候验证并缓存到localStorage的对象
